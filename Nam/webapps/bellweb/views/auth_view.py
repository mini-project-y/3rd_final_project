...
def register():
    ...
    if request.method.lower() == 'post' and form.validate_on_submit(): # validate_on_submit: 지정된 유효성 검사 규칙에 적합한지 확인하는 도구
        ...
        auth_util.insert_member(form.memberid.data, passwd_hash, form.email.data, form.gender.data, form.nationality.data, form.vegetarian_type.data, ','.join(form.excluded_meats.data), form.allergic_foods.data, ','.join(form.favorite_taste.data), ','.join(form.disliked_taste.data),  ','.join(form.preferred_taste.data), ','.join(form.non_preferred_taste.data), form.favorite_food.data, form.disliked_food.data, form.preferred_food.data, form.non_preferred_food.data)
        # return "test"
