...
def register():
    ...
    if request.method.lower() == 'post' and form.validate_on_submit(): # validate_on_submit: 지정된 유효성 검사 규칙에 적합한지 확인하는 도구
        ...
        auth_util.insert_member(form.memberid.data, passwd_hash, form.email.data, form.gender.data, form.nationality.data, form.vegetarian_type.data, form.allergic_foods.data, form.favorite_food.data, form.disliked_food.data, form.preferred_food.data, form.non_preferred_food.data)
        for ft in form.favorite_taste.data:
            auth_util.insert_member_favorite_taste(form.memberid.data, ft)
        for dt in form.disliked_taste.data:
            auth_util.insert_member_disliked_taste(form.memberid.data, dt)
        for pt in form.preferred_taste.data:
            auth_util.insert_member_preferred_taste(form.memberid.data, pt)
        for nt in form.non_preferred_taste.data:
            auth_util.insert_member_non_preferred_taste(form.memberid.data, nt)
        for em in form.excluded_meat.data:
            auth_util.insert_member_excluded_meat(form.memberid.data, em)
        # return "test"
...
# ','.join(form.excluded_meat.data), ','.join(form.favorite_taste.data), ','.join(form.disliked_taste.data),  ','.join(form.preferred_taste.data), ','.join(form.non_preferred_taste.data), 
