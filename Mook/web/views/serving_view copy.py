...
@serving_bp.route("/translate/", methods=["POST"])
def translate():
    data = request.json
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a professional translator. Preserve the original format, including line breaks, bullet points, and numbering."},
            {"role": "user", "content": f"Translate the following Korean text into English while maintaining its original format:\n\n{korean_text}"},
            {"role": "system", "content": "Translate while maintaining the original format, including line breaks, bullet points, numbering, and section headers in Markdown."}
        ]
    )
